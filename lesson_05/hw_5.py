...
'Nissan Titan': ('silver', 2018, 5.6, 'pickup', 35000) # 'ключ' (значення) значеня (specs) — це кортеж с характерстиками: silver', 2018, 5.6, 'pickup', 35000
}


search_criteria = (2017, 1.6, 36000) # tuple со свойствами в фиксированном порядке


# 1) розпакоука критеріїв
year_min, engine_min, price_max = search_criteria


# 2) Фільтрація під критерії + розгортання значень у плоску структуру
filtered = [ # ------------------------------------------------------------[ выражение (что добавляем в результат)  for элемент in коллекция(по чём итерируемся)  if условие (фильтруем элементы) ]

(brand, *specs) # (brand, color, year, engine, type, price)
for brand, specs in car_data.items()
if specs[1] >= year_min and specs[2] >= engine_min and specs[4] <= price_max
]


# 3) Сортування за зростанням ціни (x[-1] — це price)
sorted_by_price = sorted(filtered, key=lambda x: x[-1]) # функция sorted, key - ключ сортировки || сортируем по цене т.к. -1 это первое с конца спек


# 4) Вибираємо до 5 перших
top5 = sorted_by_price[:5]


# 5) Вивід
for brand, color, year, engine, car_type, price in top5:
    print(f"{brand}: {year}, {engine}L, {car_type}, {color}, ${price:,}")